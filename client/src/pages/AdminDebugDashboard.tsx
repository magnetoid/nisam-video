...
export default function AdminDebugDashboard() {
  ...
  const handleAnalyzeError = async (error: ErrorEvent) => {
    ...
    setAiAnalysis(null);
    try {
      // Mock AI analysis for now, or connect to actual endpoint if available
      // In a real implementation, this would call an endpoint that uses Ollama
      ...
      setAiAnalysis(`
        ...
        **Suggested Fix:**
        Check the connection pool settings in \`db.ts\` or validate the input payload in the controller.
      `);
    } catch (e) {
  ...
  const { data: clientLogs = [], isLoading: clientLogsLoading } = useQuery<ActivityLog[]>({
    ...
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Monitor system performance, server errors, and client-side issues
          </p>
        </div>
        <Button variant="outline" onClick={() => { refetchHealth(); refetchErrors(); queryClient.invalidateQueries({ queryKey: ["/api/activity-logs"] }); }}>
      ...
      <Tabs defaultValue="system" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {health?.criticalErrorsLastHour || 0}
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {health?.memory.rss || "0MB"}
              </div>
              <p className="text-xs text-muted-foreground">
                Heap: {health?.memory.heapUsed || "0MB"}
              </p>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Card className="col-span-1 h-[600px] flex flex-col">
            <CardHeader>
              <CardTitle>Live Error Feed</CardTitle>
          ...
          {/* Performance / Debug Tools Placeholder */}
          <div className="space-y-6">
             ...
             <Card>
                <CardHeader>
                    <CardTitle>Vibe Coding Tools</CardTitle>
                    <CardDescription>Utilities to speed up your workflow</CardDescription>
                </CardHeader>
            ...
            {selectedError && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <span className="text-muted-foreground">Module:</span> {selectedError.module}
                    </div>
                    <div>
                        <span className="text-muted-foreground">URL:</span> {selectedError.url}
                    </div>
                    <div>
                        <span className="text-muted-foreground">Type:</span> {selectedError.type}
                    </div>
                    <div>
                        <span className="text-muted-foreground">Count:</span> {selectedError.count}
                    </div>
                </div>
